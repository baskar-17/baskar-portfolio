          ...
          <div className="w-full max-w-sm transition-all duration-500 relative">
            ...
            <div className={`transition-all duration-500 border rounded-3xl ${
              stage === "high"
                ...
                : stage === "medium"
                ? "p-5 bg-stone-100 border-stone-200 shadow-xl text-stone-700 space-y-4"
                : "p-7 glass-card bg-white/90 border-black/[0.08] hover:border-black/[0.15] shadow-2xl shadow-black/[0.04] text-black space-y-5"
            }`}>
                ...
                <div>
                  <h3 className={`font-semibold tracking-tight transition-all ${
                    stage === "high" 
                      ...
                      : "text-lg font-bricolage font-bold text-black"
                  }`}>
                    {stage === "high" ? "USER CHECKOUT PANEL v1.2" : "Secure Checkout"}
                  </h3>
                  {stage !== "high" && (
                    <p className={`text-[10px] ${stage === "medium" ? "text-stone-500" : "text-black/50"} font-mono`}>
                      Invoice ID: #SG-9842
                    </p>
                  )}
                </div>
              ...
              <div className={`transition-all rounded-2xl ${
                stage === "high"
                  ...
                  : stage === "medium"
                  ? "p-4 bg-stone-200/50 border border-stone-200"
                  : "p-4 bg-black/[0.02] border border-black/5"
              }`}>
                <div className="flex justify-between items-center">
                  ...
                  <span className={`font-semibold ${
                    stage === "high" 
                      ? "text-xs text-white" 
                      : stage === "medium"
                      ? "text-sm text-stone-900 font-bold"
                      : "text-sm text-black"
                  }`}>
                    $49.00<span className={`text-[10px] font-normal ${stage === "medium" ? "text-stone-500" : "text-black/50"}`}>/mo</span>
                  </span>
                </div>
                {stage !== "high" && (
                  <div className={`mt-2 pt-2 border-t flex justify-between text-[11px] font-mono ${
                    stage === "medium" ? "border-stone-300 text-stone-500" : "border-black/5 text-black/50"
                  }`}>
              ...
              <div className="space-y-2">
                <label className={`block font-medium ${
                  stage === "high" 
                    ...
                    : "text-xs font-mono text-black/70"
                }`}>
                  Cardholder Name
                </label>
                <input
                  ...
                  className={`w-full text-xs outline-none transition-all ${
                    stage === "high"
                      ...
                      : stage === "medium"
                      ? "p-3 bg-stone-200/50 border border-stone-200 rounded-xl text-stone-800"
                      : "p-3 bg-black/[0.01] border border-black/5 hover:border-black/10 rounded-xl text-black font-geist"
                  }`}
                />
              </div>
              ...
              <div className={`flex gap-3 transition-all ${
                ...
                <button className={`w-full font-medium transition-all ${
                  stage === "high"
                    ...
                    : stage === "medium"
                    ? "py-2.5 rounded-xl border border-stone-300 hover:bg-stone-200/50 text-xs text-stone-700"
                    : "py-3 rounded-xl border border-black/10 hover:bg-black/5 text-xs text-black/70 hover:text-black font-mono"
                }`}>
                  Cancel Order
                </button>
                <button className={`w-full font-semibold transition-transform active:scale-95 flex items-center justify-center gap-1.5 ${
                  stage === "high"
                    ...
                    : stage === "medium"
                    ? "py-2.5 rounded-xl bg-stone-800 text-white hover:bg-stone-700 text-xs font-bold"
                    : "py-3 rounded-xl bg-black text-white hover:bg-neutral-800 text-xs font-mono shadow-lg shadow-black/5"
                }`}>
            ...
            {stage === "optimized" && (
              <>
                ...
                <div className="absolute -left-6 top-1/2 -translate-y-1/2 flex items-center gap-1 opacity-60">
                  <div className="h-0.5 w-4 bg-black/20" />
                  <span className="text-[8px] font-mono text-black/50 bg-white px-1 py-0.5 rounded border border-black/5 shadow-sm">p-7</span>
                </div>
                {/* Visual hierarchy alignment label */}
                <div className="absolute -right-6 top-12 flex items-center gap-1 opacity-60">
                  <span className="text-[8px] font-mono text-black/50 bg-white px-1 py-0.5 rounded border border-black/5 shadow-sm">rounded-3xl</span>
                  <div className="h-0.5 w-4 bg-black/20" />
                </div>
              </>
            )}
